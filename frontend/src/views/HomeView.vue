<template>
  <div class="min-h-screen" style="background: #f5f7fa">
    ...
    <!-- Баннер показывается только на главной (без выбранной категории) -->
    <div v-if="!selectedCategory" class="mb-0 relative banner-wrapper">
    ...
    <div class="max-w-screen-xl mx-auto px-4 relative z-10 pb-16">
      <section class="space-y-6" style="margin-top: 1rem">
        <Transition name="fade" mode="out-in">
          ...
          <div v-else key="category-focus" class="liquid-category-header">
      ...
      <section v-if="showProducts" class="mt-12">
        <div class="flex items-center justify-between mb-6">
          <button
            class="back-chip"
            @click="
              catalogStore.activeGroup ? backToGroups() : backToCategories()
            "
          >
            <svg
              width="18"
  ...
  return categories.value.map((category) => {
    ...
    const visibleGroups =
      category.groups?.filter(
        (group) => (group.totalProductCount ?? group.productCount ?? 0) > 0,
      ) ?? [];
    return {
      id: category.id,
      slug: category.slug,
...
const selectedCategory = computed<Category | null>(() => {
  ...
  return (
    categories.value.find(
      (category) => category.slug === catalogStore.activeCategory,
    ) ?? null
  );
});
...
const selectedCategoryCard = computed(() => {
  ...
  return (
    categoryCards.value.find(
      (card) => card.slug === selectedCategory.value?.slug,
    ) ?? null
  );
});
...
const liquidFallbackImage = computed(
  () => selectedCategory.value?.coverImage || PLACEHOLDER_IMAGE,
);
...
const categoryFocusStyle = computed(() => {
  const image =
    selectedCategoryCard.value?.previewImage ||
...
.main-title-adaptive {
  font-family:
    "Bebas Neue Local", "Bebas Neue", "Impact", "Arial Black", sans-serif !important;
  font-size: 2.5rem;
  max-width: 90%;
  margin-left: auto;
  margin-right: auto;
...
:deep(.liquid-product-header) {
  ...
  grid-template-columns:
    minmax(0, 2.4fr) minmax(0, 1fr) minmax(120px, 0.8fr)
    minmax(120px, 0.9fr);
  gap: 1rem;
...
/* Liquid Showcase Container - Figma Redesign */
