          ...
          >
            ...
            >
              <div class="liquid-group-meta">
                <span class="liquid-group-title">{{ group.name }}</span>
                <span class="liquid-group-count">{{ group.products.length }} вкусов</span>
              </div>
              <span class="liquid-group-icon" aria-hidden="true">
                <svg v-if="isGroupExpanded(group.id)" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <transition name="liquid-collapse">
              <div v-show="isGroupExpanded(group.id)" class="liquid-group-body">
                <ul class="liquid-product-list">
                  ...
                  >
                    <div class="liquid-product-info">
                      <span class="liquid-product-title">{{ product.title }}</span>
                      <span v-if="product.variant" class="liquid-product-variant">{{ product.variant }}</span>
                      <span v-if="product.strength" class="liquid-product-strength">{{ product.strength }}</span>
                    </div>
                    <div class="liquid-product-meta">
                      <span class="liquid-product-price">{{ formatPrice(product.priceRub) }} ₽</span>
                      <span v-if="typeof product.stock === 'number'" class="liquid-product-stock" :class="{ low: isLowStock(product) }">
...
import { useCatalogStore, type Product, type Category, type CategoryGroup } from '@/stores/catalog'
...
const liquidStructure = computed(() => {
  if (!showLiquidShowcase.value || !selectedCategory.value) {
    return { groups: [] as Array<{ id: string; name: string; order: number; products: Product[] }>, ungrouped: [] as Product[] }
  }

  const category = selectedCategory.value
  const groupMap = new Map<string, { id: string; name: string; order: number; products: Product[] }>()
...
function isLowStock(product: Product) {
  ...
  if (typeof product.minStock !== 'number') return false
...
.liquid-collapse-enter-active,
.liquid-collapse-leave-active {
  transition: max-height 0.25s ease, opacity 0.2s ease;
  overflow: hidden;
}

.liquid-collapse-enter-from,
.liquid-collapse-leave-to {
  max-height: 0;
  opacity: 0;
}

.liquid-collapse-enter-to,
.liquid-collapse-leave-from {
  max-height: 800px;
  opacity: 1;
}
</style>
