                ...
                <div class="mt-4 flex items-center justify-between">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <button
                      ...
                      class="rounded-md px-3 py-1 text-xs font-semibold text-white transition"
...
const kanbanColumns = computed(() =>
  kanbanConfig.map((column) => ({
    ...column,
    orders: orders.value.filter((order) => column.statuses.includes(order.status))
  }))
)
...
function advanceButtonClass(order: Order) {
  if (order.status === 'new') return 'bg-blue-600 hover:bg-blue-700'
  if (order.status === 'in_progress') return 'bg-emerald-600 hover:bg-emerald-700'
  return 'bg-gray-400'
}
