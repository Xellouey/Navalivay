<template>
  <div class="min-h-screen bg-gray-50 px-4 py-6 sm:px-6 lg:px-8">
    ...
    >
      <div class="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
        <div class="mb-6 flex items-start justify-between gap-4">
          <div>
            <h3 class="text-2xl font-semibold text-gray-900">{{ editingProcurementId ? 'Редактирование закупки' : 'Новая закупка' }}</h3>
            <p class="mt-1 text-sm text-gray-500">Подберите товары, укажите закупочную цену и количество — себестоимость пересчитается автоматически.</p>
          </div>
          <button class="text-gray-500 transition hover:text-gray-700" @click="closeCreateModal">Закрыть</button>
        </div>
                ...
                <tbody v-if="draftItems.length" class="divide-y">
                  <tr v-for="item in draftItems" :key="item.product.id">
                    ...
                    <td class="px-4 py-3 text-right">
                      <button class="text-sm text-red-500 transition hover:text-red-700" @click="removeItem(item.product.id)">
                        Удалить
                      </button>
                    </td>
                  </tr>
    ...
    <AdminModal
      ...
      @cancel="closeDetails"
    >
      <div class="w-full max-w-3xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
        <div class="mb-6 flex items-start justify-between gap-4">
          <div>
            <h3 class="text-2xl font-semibold text-gray-900">Закупка #{{ activeProcurement?.procurement_number }}</h3>
            <p class="mt-1 text-sm text-gray-500">{{ activeProcurement?.supplier_name || 'Без поставщика' }}</p>
          </div>
          <button class="text-gray-500 transition hover:text-gray-700" @click="closeDetails">Закрыть</button>
        </div>
        ...
        <div v-else-if="activeProcurement" class="space-y-6">
          <div class="flex flex-wrap justify-end gap-2">
            <button
              ...
              class="rounded-md border border-blue-200 px-3 py-1.5 text-xs font-semibold text-blue-600 transition hover:bg-blue-50"
            ...
            <button
              ...
              class="rounded-md border border-amber-200 px-3 py-1.5 text-xs font-semibold text-amber-600 transition hover:bg-amber-50"
        ...
        <div class="space-y-3 rounded-lg border border-gray-200 bg-white/60 p-4">
          ...
          <div v-if="quickProductImages.length" class="grid gap-3 sm:grid-cols-3">
            ...
            >
              ...
              <button
                type="button"
                class="absolute top-2 right-2 rounded-full bg-white/90 px-2 py-1 text-[11px] font-semibold text-red-600 shadow hover:bg-white"
