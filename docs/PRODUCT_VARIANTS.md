...
- Цена варианта опциональна: если в БД у варианта `priceRub` задано и это число больше нуля, публичное API отдаёт эту цену в рознице; иначе для варианта используется эффективная цена строки товара. В оптовом режиме по-прежнему действует оптовая цена тира, не розничная цена варианта.
